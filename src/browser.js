...
if (isEdge || isIE11) {
  ...
  crypto.subtle.exportKey = function () {
    ...
    var key = args[1];
    return originalExportKey.apply(crypto.subtle, args).then(function (res) {
  ...
  crypto.subtle.importKey = function () {
    ...
    return originalImportKey.apply(crypto.subtle, args).then(function (res) {
      var algo = res.algorithm;
      if (algo.name === "RSASSA-PKCS1-v1_5") {
        algo.modulusLength =
          str2buf.toUint8Array(b64u.toBinaryString(jwk.n)).length * 8;
